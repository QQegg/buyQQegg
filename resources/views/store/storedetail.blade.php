    ...
    <div class="gallery" xmlns:v-bind="http://www.w3.org/1999/xhtml"
                ...
                @else
                    @foreach($comment_id as $comment_id)
                        <form style="float: left" class="delete" action="{{route('comdestroy',$comment_id)}}"
                            ...
                            <input type="hidden" name="Store_id" value="{{$product->id}}">
        ...
        @else
            ...
            @foreach($product as $product)
                ...
                <br>
                <font size="+3" style="font-family:DFKai-sb;" class="div-left">店家：<a
                            href="{{route('stodetail',$product->store_id)}}"
                            style="font-family:DFKai-sb;">{{$product->S_name}}</a>
                </font>
                <br>
            @endforeach
            <a href="{{route('stolist')}}" class="btn btn-success">返回店家列表</a>
